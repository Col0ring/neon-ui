...
export type CalendarOnlyPropsType =
  ...
  | 'hideMinutes'
  | 'hideSeconds'
export const calendarOnlyProps = disabledTimeProps.concat(
  hideTimeProps
) as CalendarOnlyPropsType[]
...
export const omitHideDisabledProps = <T extends Record<string, any>>(
  ...
  omitBy<T>(
    props,
    (_val, key) => key.startsWith('disabled') || key.startsWith('hide')
  )

export const shouldTime = (format: string) => /([Hhms])/.test(format)

export const shouldMonth = (format: string) =>
...
export function getDateMask(formatStr: string) {
  return Array.from(formatStr).map((i) => {
    return /[A-Za-z]/.exec(i) ? /[\d|A-Za-z]/ : i
  })
}
